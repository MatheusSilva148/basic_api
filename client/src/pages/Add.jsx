...
const Add = () => {
   ...
   const handleClick = async e =>{
        e.preventDefault()
        try{
            await axios.post("http://localhost:8800/filiais", filial)
            navigate("/")
        } catch(err){
            console.log(err)
        }
    ...
    return (
        <div className='form'>
            <Title order={1}>Adicionar nova Filial</Title>
            ...
            <Button variant="gradient" gradient={{ from: 'indigo', to: 'cyan' }} onClick={handleClick}>Adicionar</Button>
        </div>
    )
}
